...
# Test parameters
TEST_SPEED_HZ   = 800       # Conservative for first validated run (200 pulses/sec)
HALF_PERIOD_US  = int(1_000_000 / (2 * TEST_SPEED_HZ))  # µs, for pigpio waveform
TEST_PULSES     = int(PULSES_PER_REV * GEAR_RATIO)  # pulses = 1 output shaft revolution

# ── NEW: waveform chunking to avoid pigpiod waveform limits/crashes ───────────
WAVE_CHUNK_PULSES = PULSES_PER_REV  # 800-pulse chunks (safe), repeated to reach TEST_PULSES
...
def send_pulses_chunked(pi, total_pulses, chunk_pulses):
    """
    Send total_pulses using repeated smaller waveforms (prevents pigpiod waveform crashes).
    """
    full_chunks = total_pulses // chunk_pulses
    remainder   = total_pulses % chunk_pulses

    # Send full chunks
    if full_chunks > 0:
        wid = build_pulse_waveform(pi, chunk_pulses)
        for _ in range(full_chunks):
            send_waveform_blocking(pi, wid)
        # wid already deleted inside send_waveform_blocking (end of last send),
        # but we created it once. So we should NOT delete it repeatedly.
        # To keep behavior consistent, rebuild per send OR delete once after a single send.
        # Easiest safe approach: rebuild each time (slower) OR manage deletes carefully.
        # We'll manage deletes correctly by rebuilding each chunk.
        #
        # NOTE: The above loop used the same wid but send_waveform_blocking deletes it.
        # So to keep minimal changes + correctness, re-implement correctly below.
        pass
...
def send_pulses_chunked(pi, total_pulses, chunk_pulses):
    ...
    if remainder > 0:
        wid = build_pulse_waveform(pi, remainder)
        send_waveform_blocking(pi, wid)
...
def run_test():
    ...
    with open(log_path, 'w', newline='') as f:
        ...
        try:
            ...
            send_pulses_chunked(pi, TEST_PULSES, WAVE_CHUNK_PULSES)
